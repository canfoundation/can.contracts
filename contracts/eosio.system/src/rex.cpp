...
namespace eosiosystem {
   ...
   /**
    * @brief Checks if account satisfies voting requirement (voting for a proxy or 6 producers)
    * for buying REX
    *
   ...
   void system_contract::check_voting_requirement( const name& owner, const char* error_msg )const
   {
      ...
      check( vitr != _voters.end() && ( vitr->proxy || 6 <= vitr->producers.size() ), error_msg );
   }
